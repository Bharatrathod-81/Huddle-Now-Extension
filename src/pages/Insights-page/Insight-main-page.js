...
const InsightPage = () => {
    ...
    const [fetchedData, setFetchedData] = useState({isFetched:false, data:g});
    
    // useEffect(() => {
    //     const cookieData = {
    //         name: "cookieName12ka4",
    //         value: "cookieValue12ka 17:57",
    //         expirationDate: new Date(Date.now() + 3600 * 1000).toUTCString(), // expires in 1 hour
    //         path: "https://meet.google.com" // set the path to root
    //       };
          
    //       document.cookie = `${cookieData.name}=${cookieData.value}; expires=${cookieData.expirationDate}; path=${cookieData.path}`;
          
    
    //     // Optional: Check if the cookie was added successfully
    //     console.log(`Cookie ${cookieData.name} added successfully!`);
    //   }, []);
    ...
    return (
        <div className="flex w-full" >
            {openLeftSection && <LeftSection setSelectedMeeting={setSelectedMeeting} selectedMeeting={selectedMeeting} fetchedData={fetchedData}/>}
